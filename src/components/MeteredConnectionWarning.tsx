    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50">
      <div className="card p-6 max-w-md w-full border-2 border-orange-300 dark:border-orange-700" data-testid="metered-warning">
        <div className="flex items-start gap-4 mb-4">
          ...
          <div className="flex-1">
            <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-2">
              {t('database.meteredConnection')}
            </h2>
            <p className="text-gray-700 dark:text-gray-300 text-sm mb-3">
              <Trans
                i18nKey="database.meteredConnectionMessage"
                values={{ size: databaseSizeMB }}
                components={{ strong: <strong /> }}
              />
            </p>
            <p className="text-gray-700 dark:text-gray-300 text-sm mb-4">
              {t('database.meteredConnectionWarning')}
            </p>
            <div className="flex items-center gap-2 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
              <Wifi className="w-4 h-4 text-blue-600 dark:text-blue-400 flex-shrink-0" />
              <p className="text-xs text-blue-800 dark:text-blue-300">
                {t('database.meteredConnectionRecommend')}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          >
            {t('database.waitForWifi')}
          </button>
          <button
            onClick={onConfirm}
            className="flex-1 px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition-colors font-medium"
          >
            {t('database.downloadAnyway')}
          </button>
        </div>
      </div>
    </div>
